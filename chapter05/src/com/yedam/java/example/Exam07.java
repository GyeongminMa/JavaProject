...
public class Exam07 {
	...
	public static void main(String[] args) {
		// 로또 번호를 예측하는 프로그램 -6개
		boolean run = true; //false는 if문 못돌림
		...
		while (run) {
			System.out.println("-----------------------------------------------------------");
			System.out.println("1.새로고침 | 2.번호예측 | 3. 번호삭제 | 4. 번호 출력 | 5. 분석 |6. 종료");
			System.out.println("-----------------------------------------------------------");
			System.out.println("선택 > ");
			int selectNo = Integer.parseInt(sc.nextLine());

			if (selectNo == 1) {// 로또번호를 담을 수 이쓴 배열 초기화
				numList = new int[6];
				count = 0;
			} else if (selectNo == 2) {// 1~45 중에 랜덤한 값을 하나씩 가져옴
				if (count < numList.length) {// 무조건 작아야함
				...
				numList[--count] = 0;// - 뒤로가면안됨 위치 중요 =[(count)-1]
				System.out.println("현재 가져온 수의 갯수 : " + count);
